...
for n in range(N_COMPUTERS):
    computer = Intcode(mem.copy(), [n], False)
...
for iteration in range(200):
    ...
    no_inputs = True
    for n in range(N_COMPUTERS):
        # provide -1 to input if no inputs are present
        if len(computers[n].inputs) == 0:
            computers[n].inputs.append(-1)
        else:
            no_inputs = False
        # run
        computers[n].run()
        ...
        # consume outputs
        outputs_queue += computers[n].outputs
        computers[n].outputs = []

    # process outputs (if any)
    for address, x, y in zip(*[iter(outputs_queue)] * 3):
        # print(address,  x, y)
        if address == 255:
            print(address, x, y)
            nat = [x, y]
    ...
    # is network idle?
    if len(outputs_queue) == 0 and no_inputs:
        print("idle:", nat)
        if nat[1] in ys:
            print("FOUND", nat[1])
            exit()
