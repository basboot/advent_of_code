...
computer = Intcode(mem.copy(), [], False)

status = -1

while status != 0:
    # skip input for first run
    if status > -1:
        computer.outputs = []
        command = input('> ')
        # print("command", command)
        inputs = [ord(c) for c in list(command)]
        # print(inputs)
        computer.inputs = inputs + [10] # add enter

    status = computer.run()
    print("".join([chr(x) for x in computer.outputs]))
