...
def get_neighbours(i, j):
    ...
    for di, dj in [(-1, 0), (0, 1), (1, 0), (0, -1)]:
        tile = get_maze(i + di, j + dj)
        assert tile is not None, "Assumption . is never at the boundary is not correct"

        if tile == ".":
            neighbours.append(str((i + di, j + dj)))
        else:
            if tile.isalpha(): # portal
                next_tile = get_maze(i + 2*di, j + 2*dj)
                if (di, dj) in [(-1, 0), (0, -1)]:
                    neighbours.append(next_tile + tile) # up and left read reversed
                else:
                    neighbours.append(tile + next_tile)
        ...
        if row[j] == ".":
            for neighbour in get_neighbours(i, j):
                if neighbour[0].isalpha():
                    G.add_edge(str((i, j)), neighbour, weight=0.5) # portals length 1 for stepping in and out
                else:
                    G.add_edge(str((i, j)), neighbour, weight=1)
