...
# extend cavern
extended_cavern = np.zeros((width * 5, height * 5))
for i in range(5):
    for j in range(5):
        extended_cavern[i * height: (i+1) * height, j * width: (j+1) * width] = (((cavern + i + j - 1) % 9) + 1)

print(extended_cavern)

cavern = extended_cavern
width = width * 5
height = height * 5
